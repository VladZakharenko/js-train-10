...
function customAssign(...objects) {
  ...
  let mObj = Object.assign({}, ...objects);
  for (const i in mObj) {
    if (typeof mObj[i] === 'number') { mObj[i] = mObj[i].toString()}
  }
  return mObj
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  else {return 'Щось пішло не так. Спробуйте ще раз пізніше'}
...
function filterObjectsByKey(arr, key) {
  ...
  let arr1 = arr.filter(obj => obj.hasOwnProperty(key))
  return arr1
...
function checkOwnership(obj1, obj2, key) {
  if (obj1.hasOwnProperty(key) && obj2.hasOwnProperty(key)) {return Object.is(obj1.key, obj1.key)}
  return false
